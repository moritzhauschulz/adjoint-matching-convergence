...
def plot_inner_steps(snapshots: list[dict], output_dir: str | Path) -> None:
    """Line plot of how many inner-loop gradient steps ran per outer iteration.

    Saves to output_dir/inner_steps.png.
    """
    output_dir = Path(output_dir)
    ...
    max_steps = max(steps)
    ax.axhline(max_steps, color="gray", linewidth=1.0, linestyle="--", alpha=0.6,
               label=f"budget ({max_steps})")
    ax.set_xlabel("outer iteration", fontsize=12)
    ax.set_ylabel("inner steps taken", fontsize=12)
    ax.set_title("Inner-loop steps per outer iteration\n"
                 "(below budget = early convergence)", fontsize=11)
...
def plot_inner_convergence(snapshots: list[dict], output_dir: str | Path) -> None:
    """Multi-curve plot of the inner-loop RAM loss trajectory for selected outer iterations.

    Only outer iterations that have a recorded 'inner_loss_curve' are shown.
    Saves to output_dir/inner_convergence.png.
    """
    ...
    for i, snap in enumerate(snaps):
        curve = snap["inner_loss_curve"]
        ax.plot(range(len(curve)), curve, color=colours[i],
                label=f"outer {snap['outer_it']}", linewidth=1.2, alpha=0.85)

    ax.set_xlabel("inner step", fontsize=12)
    ax.set_ylabel("RAM loss", fontsize=12)
    ax.set_title("Inner-loop convergence across training\n"
                 "(light = early outer iterations, dark = late)", fontsize=11)
    ax.set_yscale("log")
    ax.grid(True, alpha=0.3, which="both")
...
def load_and_plot(metrics_json: str | Path, output_dir: str | Path | None = None) -> None:
    ...
    Usage:
        from experiments.right_to_left_convergence_bimodal.plotting import load_and_plot
    ...
    w1 = float(tp.get("w1", 0.5))
    w2 = float(tp.get("w2", 0.5))
    lambda1 = float(tp.get("lambda1", 1.0))
    lambda2 = float(tp.get("lambda2", 1.0))
    mu1 = float(tp.get("mu1", -1.0))
    mu2 = float(tp.get("mu2", 1.0))

    def _make_target_pdf():
        # Full objective: p^{u*}(x1) ∝ exp(r(x1)) = mixture N(μi, 1/λi) with αi ∝ wi/√λi
        a1 = w1 / math.sqrt(lambda1)
        a2 = w2 / math.sqrt(lambda2)
        norm = a1 + a2
        alpha1, alpha2 = a1 / norm, a2 / norm
        v1s, v2s = 1.0 / lambda1, 1.0 / lambda2
